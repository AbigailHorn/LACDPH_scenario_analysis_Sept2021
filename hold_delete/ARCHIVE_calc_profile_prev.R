...
### Correct by adding the 3 BMI statuses for the "Age0.19" group
...
### THIS IS WHAT I AM SAVING IN THIS ARCHIVE FILE -- SAVING THE INPUT FROM LAI
write.csv(Pr.H, file = path(data.dir, "Pr.H.csv"))
write.csv(Pr.Q, file = path(data.dir, "Pr.Q.csv"))
write.csv(Pr.D, file = path(data.dir, "Pr.D.csv"))
...
Pop.prevalence.LA <- Pop.prevalence[9,]
write.csv(Pop.prevalence.LA, file = path(data.dir, "Pop.prevalence.LA.csv"))
POPTEST <- read.csv(path(data.dir, "Pop.prevalence.LA.csv"), sep=",", header=TRUE,row.names = 1 )
...
write.csv(SIGMA2, file = path(data.dir, "SIGMA.csv"))
