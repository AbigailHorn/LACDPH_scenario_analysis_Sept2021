...
##########################################################################################
## Do in another run scenario code
## For each scenario, specify:
### protect.scenario
### scenario.NPI
## Then run that using a modified version of correlated.param.SIM, that reads in all the 
## variables[i], but then does:
## if protect.scenario=1 or 2 , Alpha1 <- ... [protect.scenario,"Alpha.t1"]
## if scenario.NPI = "moderate" or "nothing", then Beta_y comes from below
...
if (scenario.NPI == 1){ Beta_y_vals <- "Beta_y" }
if (scenario.NPI == 2) { Beta_y_vals <- "Beta_y_moderate" }
if (scenario.NPI == 3) {Beta_y_vals <- "Beta_y_nothing" }
