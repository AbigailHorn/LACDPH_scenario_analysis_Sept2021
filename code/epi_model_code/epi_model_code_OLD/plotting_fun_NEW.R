...
plot.param.t <- function(ABC_out=ABC_out, endDatePlot=endDatePlot){
  ...
  traj.S <- traj.S[c(1:min(nrow(traj.S),nrow(Rt_add))),]
  
  ###########################################################################
  ### MULTIPLYING EACH INTERVAL by S(t)/N
  
  TEST <- Rt_add
  TEST = TEST %>% plyr::mutate(low_95 = low_95* (traj.S$low_95 / 1e7)  )
  TEST = TEST %>% plyr::mutate(low_50 = low_50* (traj.S$low_50 / 1e7)  )
  TEST = TEST %>% plyr::mutate(median = median* (traj.S$median / 1e7)  )
  TEST = TEST %>% plyr::mutate(up_50 = up_50* (traj.S$up_50 / 1e7)  )
  TEST = TEST %>% plyr::mutate(up_95 = up_95* (traj.S$up_95 / 1e7)  )
  #TEST = TEST[c(1:min(nrow(traj.S),nrow(TEST))),]
  TEST$state.name = rep("Rt", times = nrow(TEST))
  
  # PLOTTING R(t)
  traj.CI <- TEST # Rt_plot
  vars.to.plot <- "Rt"
  data.in <- NULL
  y.max.in <- 4
  y.lab.in <- "R(t)"
