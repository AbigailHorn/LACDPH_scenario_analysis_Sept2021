...
plot.model.single <- function(traj.CI, data.in, init.date.data=NULL, time.steps.4plot=NULL, ymax=NULL, plot.capacity=NULL, plot.annotations=NULL, var.to.plot=NULL) {
  ...
  if (!is.null(plot.annotations)){
    ...
    traj.CI.date <- as.data.frame(matrix(NA, nrow=9, ncol=3))
    colnames(traj.CI.date) <- c("date","date.label","y.place")
    traj.CI.date$date <- c(as.Date("2020-03-19"),as.Date("2020-05-08"),as.Date("2020-06-12"),as.Date("2020-07-01"),as.Date("2020-08-18"),as.Date("2020-10-31"),as.Date("2020-11-26"),as.Date("2020-12-25"),as.Date("2021-01-01"))
    traj.CI.date$date.label <- c("Stage I", "Stage II", "Stage III", "Modifications", "School Year", "Halloween", "Thanksgiving","Christmas","New Years")
    traj.CI.date$y.place <- c(1:9)
...
plot.together.capacity <- function(traj.CI=traj.CI, data.in=data.in, endDatePlot=endDatePlot, vars.to.plot, y.lab.in, y.max.in, chart.title, plot.capacity, plot.annotations) {
  ...
  if(!is.null(data.in)){
    ...
    if(any(vars.to.plot %in% colnames(data.in))) {  
  ...
  if (!is.null(plot.capacity)){
    ...
    p <- p + geom_hline(data = capacity.vals, aes(yintercept=capacity),linetype = "dotted", colour="azure4")
  }
...
plot.param.t <- function(ABC_out=ABC_out, endDatePlot=endDatePlot){
  ...
  var.to.plot <- "Rt_eff"
  R_t_eff_plot <- plot.model.single(traj.CI, data.in=NULL, init.date.data=NULL, time.steps.4plot=time.steps.4plot, ymax=4, plot.capacity=NULL, plot.annotations=TRUE, var.to.plot=var.to.plot)
  R_t_eff_plot
  
  Rt_add$state.name = rep("Rt", times=nrow(Rt_add))
  traj.CI <- Rt_add
  var.to.plot <- "Rt"  
  time.steps.4plot <- as.numeric(as.Date(endDatePlot) - as.Date("2020-03-01"))
  ...
  plot.out[[1]] <- R_t_plot
  plot.out[[2]] <- R_t_eff_plot
  plot.out[[3]] <- r_t_plot
  plot.out[[4]] <- AKD_t_plot
...
plot.out.compartmental <- function(traj.CI, endDatePlot){
  ...
  traj.CI <- traj.0
  data.in <- la_data
  y.max.in <- traj.CI %>% filter(state.name=="Htot") %>% select(up_95) %>% max() %>% round(digits=-3) + 500
  y.lab.in <- "Current Census in Compartment"
  vars.to.plot<-c("Q", "Htot")
  ...
  traj.CI <- traj.0
  data.in <- la_data
  y.max.in <- 1e6 #traj.CI %>% filter(state.name=="Itot") %>% select(up_95) %>% max() %>% round(digits=-5) + 50000
...
########################################################################################
## PLOTTING PRIOR AND POSTERIOR VARIABLES
########################################################################################

plot_prior_posterior <- function(ABC.density, PAR, PAR.name, PAR.mean, PAR.sd){
  # Posterior plot
  posterior_plot <- ggplot(ABC.density, aes(x=ABC.density[,PAR])) + geom_density() + xlab(PAR.name) + ggtitle(paste0(PAR.name, " Posterior Distribution"))
  # Prior plot
  x=seq(  min(ABC.density[,PAR]) - min(ABC.density[,PAR])*0 , max(ABC.density[,PAR])+max(ABC.density[,PAR])*0 ,by= (max(ABC.density[,PAR])-  min(ABC.density[,PAR]))/100)
  priorDensity=as.data.frame(cbind(x, dnorm(x, mean = PAR.mean, sd = PAR.sd)))
  colnames(priorDensity) <- c("x","density")
  prior_plot <- ggplot(priorDensity, aes(x=x,y=density)) + geom_line() + ggtitle(paste0(PAR.name, " Prior Distribution"))
  # Print
