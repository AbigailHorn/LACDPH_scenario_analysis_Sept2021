...
risk_table_CFR_FULL <- risk.table.CFR.IFR.dates(ABC.out.mat=ABC.out.mat[1:100,], time.steps=time.steps, iter=iter, data.prev=data.prev, times.dates=times.dates, round.by=round.by)
...
risk_table_CFR <- risk.table.CFR.IFR.dates(ABC.out.mat=ABC.out.mat, time.steps=time.steps, iter=iter, data.prev=data.prev, round.by=round.by)
risk_table_CFR <- risk_table_CFR[,c("Profile","riskprofile","age","comorbidity","BMI","smoking","freq.PREV.q", paste0("freq.I.",times),            
                                    paste0("CFR.",times),paste0("IFR.",times))]
#vars <- c("freq.I.","CFR.","IFR.")
#apply(expand.grid(vars, times), 1, paste, collapse="")
#risk_table_CFR <- filter(risk_table_CFR, "I.prev.t1" > 0.000000004)  # FILTER PROFILES TO SHOW IN THE TABLE TO ONLY THOSE > 0 PREVALENCE
risk_table_CFR <- arrange(risk_table_CFR, desc(risk_table_CFR$"CFR.t1"))
risk_table_CFR <- risk_table_CFR %>% mutate_if(is.numeric, round, digits=5)
colnames(risk_table_CFR)[colnames(risk_table_CFR)=="freq.PREV.q"] <- "Pop.Prev"
colnames(risk_table_CFR)[colnames(risk_table_CFR)=="riskprofile"] <- "Group"
colnames(risk_table_CFR)[colnames(risk_table_CFR)==paste0("freq.I.",times)] <- paste0("Inf.",times.dates)
colnames(risk_table_CFR) <- c(colnames(risk_table_CFR)[1:7], paste0("Inf.",table.dates), paste0("CFR.",table.dates), paste0("IFR.",table.dates))
risk_table_CFR <- filter(risk_table_CFR, "Pop.Prev" > 0.000001)  # FILTER PROFILES TO SHOW IN THE TABLE TO ONLY THOSE > 0 PREVALENCE
...
risk_table_CFR.ft <- formattable(risk_table_CFR, align="c",
  ...
  `Group`= color_tile('red', 'green'),
  `Pop.Prev`= color_bar('cornflowerblue', fun = unit.scale),
    `Inf.Apr.15`= color_bar('mediumorchid', fun = unit.scale),
  `Inf.Jul.15`= color_bar('violet', fun = unit.scale),
  `Inf.Oct.15`= color_bar('lightpink', fun = unit.scale),
  `CFR.Apr.15`= color_bar('mediumorchid', fun = unit.scale),
  `CFR.Jul.15`= color_bar('violet', fun = unit.scale),
  `CFR.Oct.15`= color_bar('lightpink', fun = unit.scale),
  `IFR.Apr.15`= color_bar('mediumorchid', fun = unit.scale),
  `IFR.Jul.15`= color_bar('violet', fun = unit.scale),
  `IFR.Oct.15`= color_bar('lightpink', fun = unit.scale)
))
risk_table_CFR.ft
...
# export_formattable <- function(f, file, width = "900", height = "800",#, 
#                                background = "white", delay = 0.2)
#     {
#       w <- as.htmlwidget(f, width = "100%", height = NULL)
#       path <- html_print(w, background = background, viewer = NULL)
#       url <- paste0("file:///", gsub("\\\\", "/", normalizePath(path)))
#       webshot(url,
#               file = file,
...
#     viewer.padding = 0,
#     viewer.paneHeight = 'maximize',
#     #viewer.paneWidth = 'maximize',
#     browser.fill = TRUE
#   ))
# 
# as.htmlwidget(
