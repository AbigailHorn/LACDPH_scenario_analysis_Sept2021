...
n.dates.LAC.obs.age <- 3
freq.LAC.obs.age <- matrix(nrow=3, ncol=n.dates.LAC.obs.age)
colnames(freq.LAC.obs.age) <- c("t1","t2","t3")
...
freq.LAC.obs.age[,3] <- c(0.1, 0.8, 0.1)
...
table2 <- as.data.frame(matrix(0,nrow=2,ncol=3))
table2[1,] <- as.numeric(risk.probs.POSTERIORS[1,])
table2[2,] <- as.numeric(risk.probs.POSTERIORS[2,])
table2[3,] <- table2[2,]
colnames(table2) <- colnames(risk.probs.POSTERIORS)
rownames(table2) <- c("t1","t2","t3")
risk.probs.POSTERIORS <- table2
