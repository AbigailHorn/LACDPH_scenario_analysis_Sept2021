...
if (print.output==TRUE){
  ...
  p.prev.risk.stage.DATES + p.prev.age.stage.DATES + p.prev.BMI.stage.DATES +  
    p.prev.comorbidity.stage.DATES + p.prev.smoking.stage.DATES + plot_annotation(tag_levels = 'A' )
  dev.off()
}
...
ABC.out.mat.scenarios = ABC.out.mat[c(1:100),]
iter=10
time.steps=400
...
if (print.output==TRUE){
  pdf(file = path(output.dir, "plot.SCENARIOS.ALL"), width=10, height =10)
  plot.SCENARIOS.ALL
  dev.off()
  
  pdf(file = path(output.dir, "plot.SCENARIOS.filter"), width=10, height =10)
