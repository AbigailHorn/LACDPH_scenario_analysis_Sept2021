...
print.output <- TRUE
days.truncate <- 0
endDatePlotGlobal <- as.Date("2022-01-01")
...
if (print.output==TRUE) write.csv(tables.out[[4]], file = path(output.dir, "epi_model_output/CFR_IFR_table_pct.csv"))
```
...
RR.age.table <- exp(psi.mat) %>% round(digits=2)
write.csv(RR.age.table, path(output.dir, "risk_model_output/RR.age.table.csv"))
...
#if (print.output==TRUE) {
  write.csv(CFR.CI.table, file = path(output.dir, "risk_model_output/CFR.tables/CFR_pct.csv"))
  write.csv(IFR.CI.table, file = path(output.dir, "risk_model_output/CFR.tables/IFR_pct.csv"))
#}
...
```{r}
p <- (p.prev.age.stage.DATES + p.prev.comorbidity.stage.DATES) / (p.prev.BMI.stage.DATES + p.prev.smoking.stage.DATES) +  plot_annotation(tag_levels = 'A') & 
  theme(plot.tag = element_text(size = 15))

p

pdf(file = path(output.dir, "risk_model_output/risk.stages.DATES.pdf"), width=12, height =12)
p
dev.off()
```
